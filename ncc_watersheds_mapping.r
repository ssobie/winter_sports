...
make_watersheds_plot <- function(var.name,plot.type,var.title,plot.data,plot.file,class.breaks=NULL,mark,leg.title) {
   ...
   lons <- c(-123.25, -123.0,-122.75,-122.5)
   lats <- c(49.0, 49.25, 49.5, 49.75, 50.0, 50.25, 50.5, 50.75)

   shape.dir <- '/storage/data/projects/rci/data/assessments/shapefiles/bc'
   bc.shp <- get.region.shape('bc',shape.dir)

   map.extent <- extent(c(-123.4,-122.4,49.01,49.8))
...
##   plot(spTransform(region.shp,CRS(alb.crs)),add=TRUE,border='black',cex=0.25)
