...
slen <- 100
    ...
    png(file=paste0(plot.dir,model,'.SWE.normalized.pillow.series.1001.png'),width=1000,height=900)
...
for (i in seq_along(sites)) {
    ...
    flag <- is.na(pillow.swe)
    sims <- apply(swe.sims,1,mean)[date.subset]
    asp <- pillow.swe

    sims.na <- sims[!flag]
    asp.na <- pillow.swe[!flag]
    ...
    if (1==0) {
      ...
      plot(as.Date(dates[date.subset]),apply(swe.sim.norms[date.subset,],1,mean),xlab='Date',ylab='SWE (mm)',yaxs='i',
           ...
           xlim=c(as.Date('1992-06-01'),as.Date('2011-01-01')),ylim=c(-2,6))
