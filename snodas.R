MkNcdf <- function (varList, filename, globalAttList = NULL, overwrite = FALSE, 
    force_v4 = TRUE, compLev = -1) 
{
    ClassMapper <- c(numeric = "float", integer = "integer", 
        character = "text")
    names(varList) <- plyr::laply(varList, "[[", "name")
    fileExists <- file.exists(filename)
    if (!fileExists | overwrite) {
        append <- FALSE
        doDefVar <- function(var) {
            doDimDef <- function(dim) {
                ...
                ncdf4::ncdim_def(name = dim$name, units = dim$units, 
                  vals = dim$values, unlim = dim$unlim, create_dimvar = dim$create_dimvar)
            }
            dimList <- plyr::llply(var$dimensionList, doDimDef)
            if (!length(var$precision)) 
                var$precision <- ClassMapper[class(var$data[1])]
            if (compLev == -1) {
                ncdf4::ncvar_def(var$name, var$units, dimList, 
                  var$missing, longname = var$longname, prec = var$precision)
            }
            else {
                ncdf4::ncvar_def(var$name, var$units, dimList, 
                  var$missing, longname = var$longname, prec = var$precision, 
        ...
        newVarsUnlim <- plyr::laply(names(varList), function(vv) ncid$var[[vv]]$unlim)
        if (!all(newVarsUnlim)) 
    ...
    for (varName in names(varList)) {
        var <- varList[[varName]]
        if (append) {
            start <- (ncid$var[[var$name]]$varsize * plyr::laply(ncid$var[[var$name]]$dim, 
                "[[", "unlim")) + 1
            count <- plyr::laply(varList[[var$name]]$dimensionList, 
                function(dd) length(dd$values))
            varDimInfo <- ncid$var[[var$name]]$dim
            names(varDimInfo) <- plyr::laply(varDimInfo, "[[", 
                "name")
            unlimDimName <- subset(plyr::ldply(varDimInfo, "[[", 
                "unlim"), `[[`)$.id
            ncdf4::ncvar_put(ncid, unlimDimName, var$dimensionList[[unlimDimName]]$values, 
                start = varDimInfo[[unlimDimName]]$len + 1, count = length(var$dimensionList[[unlimDimName]]$values))
            ncdf4::ncvar_put(ncid, var$name, var$data, start = start, 
                count = count)
        }
        else {
    ...
    if (!is.null(globalAttList)) {
        doPutGlobalAtts <- function(att) ncdf4::ncatt_put(ncid, 
            0, att$name, att$value, prec = att$precision)
...
FormalsToDf <- function (theFunc, envir = parent.frame()) 
{
    ...
    formalLens <- plyr::laply(theFormals, function(ff) length(get(ff, 
        envir = envir)))
    ...
    sortFormals <- theFormals[sort(formalLens, index = TRUE, 
        decreasing = TRUE)$ix]
...
newGetSnodasDepthSweDate <- function(datePOSIXct, outputDir='.', overwrite=FALSE, 
                                           quiet=TRUE) {
    ...
    theFile <- paste0('SNODAS_unmasked_',yy,mm,dd,'.tar')
    print(paste0('File: ',theFile))
    # if theFile (tarball) exists, then skip downloading unless told to overwrite

    if(!file.exists(theFile) | overwrite) {
      theUrl <- paste0('ftp://sidads.colorado.edu/DATASETS/NOAA/G02158/unmasked/',
                       yy,'/',mm,'_',mon,'/',theFile)
