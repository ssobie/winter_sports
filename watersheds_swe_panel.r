...
source('/storage/home/ssobie/code/repos/winter_sports/ncc_watersheds_panel_map.r',chdir=T)
...
class.breaks <- c(0,100,200,300,400,500,600,700,800,1000,1250,1500,2000,2500,3000,3500) ##Winter Mean
...
rv <- make_watershed_panel_plot(var.name='swe',plot.type='past',plot.title,past.ens.avg,
                                class.breaks=class.breaks,y.axis=TRUE,letter='A')
##contour(x=lon,y=lat,z=past.matrix,levels=100,lwd=1.5,add=T,labels='')

rv <- make_watershed_panel_plot(var.name='swe',plot.type='past',plot.title,
                               proj.ens.one,class.breaks=class.breaks,letter='B',
                               add.legend=TRUE,leg.title='SWE (mm)')
##contour(x=lon,y=lat,z=past.matrix,levels=100,lwd=0.75,add=T,lty=2,labels='')
##contour(x=lon,y=lat,z=one.matrix,levels=100,lwd=1.5,add=T,labels='')

rv <- make_watershed_panel_plot(var.name='swe',plot.type='past',plot.title,
                               proj.ens.two, class.breaks=class.breaks,
                               x.axis=TRUE,y.axis=TRUE,letter='C')
##contour(x=lon,y=lat,z=past.matrix,levels=100,lwd=0.75,add=T,lty=2,labels='')
##contour(x=lon,y=lat,z=two.matrix,levels=100,lwd=1.5,add=T,labels='')

rv <- make_watershed_panel_plot(var.name='swe',plot.type='past',plot.title,
                       proj.ens.three,
                       class.breaks=class.breaks,x.axis=TRUE,letter='D')
##contour(x=lon,y=lat,z=past.matrix,levels=100,lwd=0.75,add=T,lty=2,labels='')
##contour(x=lon,y=lat,z=three.matrix,levels=100,lwd=1.5,add=T,labels='')
