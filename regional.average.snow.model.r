...
area_average_swe_series <- function(gcm,var.name,swe.file,swe.dir,clip.shp) {
   ...
   for (y in seq_along(years)) { 
      ...
      swe.mask <- mask(swe.sub,clip.shp)
...
##   gcm <- gcm.list[i]
   ...
   swe.info <-  area_average_swe_series(gcm,var.name,swe.file,tmp.dir,clip.shp)
   ...
   write.table(output,
               file=paste('/storage/data/projects/rci/data/winter_sports/BCCAQ2/TPS/snow_series/',
                           'mv_watersheds_',gcm,'_PNWNAmet_PRISM_TPS_snow_model_data.csv',sep=''),
