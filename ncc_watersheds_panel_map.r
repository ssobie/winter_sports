...
make_watershed_panel_plot <- function(var.name,plot.type,var.title,plot.data,class.breaks=NULL,
   ...
   lons <- c(-123.4, -123.2,-123.0,-122.8,-122.6,-122.4)
   lats <- c(49.2, 49.3,49.4,49.5, 49.6,49.7)
   latsC <- c(49.2, 49.3,49.4,49.5, 49.6)

   shape.dir <- '/storage/data/projects/rci/data/assessments/shapefiles/bc'
   bc.shp <- get.region.shape('bc',shape.dir)

   map.extent <- extent(c(-123.375,-122.5,49.2,49.75))
   ...
   if (y.axis) {
     if (letter=='A') {axis(2,at=lats,label=lats,cex.axis=2.25,mgp=c(4,1.5,0))}
     if (letter=='C') {
        axis(2,at=lats,label=rep('',length(lats)),cex.axis=2.25,mgp=c(4,1.5,0))
        axis(2,at=latsC,label=latsC,cex.axis=2.25,mgp=c(4,1.5,0))
     }
   }
   if (x.axis) {
   ...
   watersheds.shp <- spTransform(get.region.shape('MVWaterSheds',shape.dir),CRS("+init=epsg:4326")) 
   ...
   plot(spTransform(watersheds.shp,CRS(alb.crs)),add=TRUE,border='black',lwd=2)
   plot(spTransform(wash.shp,CRS(alb.crs)),add=TRUE,col='gray',border='black',cex=0.5)
   text(-123.3,49.28,letter,cex=3)
